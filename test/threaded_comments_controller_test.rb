...
class ThreadedCommentsControllerTest < ActionController::TestCase 
  def setup 
    ...
    ActionController::Routing::Routes.draw do |map| 
      map.flag_threaded_comment          '/threaded_comments/:id/flag', :controller => 'threaded_comments', :action => 'flag', :conditions => { :method => :post }
      map.upmod_threaded_comment         '/threaded_comments/:id/upmod', :controller => 'threaded_comments', :action => 'upmod', :conditions => { :method => :post }
      map.downmod_threaded_comment       '/threaded_comments/:id/downmod', :controller => 'threaded_comments', :action => 'downmod', :conditions => { :method => :post }
      map.create_threaded_comment        '/threaded_comments', :controller => 'threaded_comments', :action => 'create', :conditions => { :method => :post }
      map.remove_threaded_comment_email  '/threaded_comments/:id/remove-email/:hash', :controller => 'threaded_comments', :action => 'remove_email' 
    end  
  ...
  def test_should_only_allow_voting_or_flagging_once_per_session
    ...
    @actions.each do |action|
      assert_difference("Comment.find(1).#{action[:field]}",action[:difference], "Action failed first time: #{action[:action]}") do
        put action[:action], :id => 1
        assert_redirected_to rant_path(Comment.find(1).rant)
      end 
      assert_no_difference( "Comment.find(1).#{action[:field]}", "Action succeeded when it should have failed: #{action[:action]}") do
  ...
  def test_should_not_remove_email_address_if_hash_does_not_match
    test_comment = ThreadedComment.find(1)
    assert !test_comment.email.empty?
    assert test_comment.notifications == true
    get :remove_email, :id => 1, :hash => test_comment.email_hash + "1"
    assert_response :success
    test_comment.reload
    assert !test_comment.email.empty?
    assert test_comment.notifications == true
