...
Upgrading from a previous version of has_threaded_comments is easy. Simply re-perform
the steps outlined in the section marked 'installation' above. When you run the 'install_has_threaded_comments'
generator again it will ask you if you would like to replace your existing config files with the new versions.
Make sure you backup any changes you made to your config file or stylesheet so that you can add
them back into the new files.

=== More info
 
Please refer to {MORE_INFO.rdoc}[link:files/MORE_INFO_rdoc.html]
